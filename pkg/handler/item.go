...
func (h *Handler) updateItem(c *gin.Context) {
	...
	var input todoServer.UpdateItemInput
	if err := c.BindJSON(&input); err != nil {
		newErrorResponse(c, http.StatusBadRequest, err.Error())
		return
	}

	err = h.services.UpdateItem(itemId, userId, input) //мб указатель?
	if err != nil {
		newErrorResponse(c, http.StatusBadRequest, err.Error())
		return
	}
...
func (h *Handler) deleteItem(c *gin.Context) {
	...
	err = h.services.DeleteItem(itemId, userId) //мб указатель?
	if err != nil {
		newErrorResponse(c, http.StatusBadRequest, err.Error())
		return
	}
	c.JSON(http.StatusNoContent, responseStatus{Status: "deleted"})
}
